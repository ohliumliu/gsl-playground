...
#define FUNC4(x) test_ ## x, test_ ## x ## _cdf, "test gsl_ran_" #x /* use cdf to calculate expected count */
...
/* a separate function to use cdf when calculating expected counts in each bin 
Use testPDF_c instead to avoid too much code repetition?
*/
void
...
/* this is essentially testPDF with an extra flag to indicate whether to pdf is actually a cdf.
  If cdf == 1, use pdf as a cdf to calculate expected counts */
void
